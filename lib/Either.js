...
function getEq(SL, SA) {
    return Eq_1.fromEquals(function (x, y) { return (isLeft(x) ? isLeft(y) && SL.equals(x.left, y.left) : isRight(y) && SA.equals(x.right, y.right)); });
}
exports.getEq = getEq;
/**
...
function filterOrElseL(ma, predicate, zero) {
    return isLeft(ma) ? ma : predicate(ma.right) ? ma : left(zero(ma.right));
}
exports.filterOrElseL = filterOrElseL;
/**
...
function getFilterable(M) {
    ...
    var filter = function (ma, p) { return filterOrElse(ma, p, M.empty); };
