...
function getEq(EL, EA) {
    return Eq_1.fromEquals(function (x, y) {
        return isLeft(x) ? isLeft(y) && EL.equals(x.left, y.left) : isRight(y) && EA.equals(x.right, y.right);
    });
}
exports.getEq = getEq;
/**
 ...
 */
function orElse(f) {
    return function (ma) { return (isLeft(ma) ? f(ma.left) : ma); };
}
...
function getOrElse(f) {
    return function (ma) { return (isLeft(ma) ? f(ma.left) : ma.right); };
}
...
exports.elem = elem;
function filterOrElse(predicate, zero) {
    return function (ma) { return (isLeft(ma) ? ma : predicate(ma.right) ? ma : left(zero(ma.right))); };
}
...
function getValidationApplicative(S) {
    return {
        ...
        extend: exports.either.extend,
        bimap: exports.either.bimap,
        mapLeft: exports.either.mapLeft
    };
}
 ...
 * @since 2.0.0
 */
function getValidationMonad(S) {
    return __assign({}, getValidationApplicative(S), { chain: function (ma, f) { return (isLeft(ma) ? ma : f(ma.right)); } });
}
exports.getValidationMonad = getValidationMonad;
/**
 * @since 2.0.0
 */
...
function getValidationMonoid(SE, SA) {
    return __assign({}, getValidationSemigroup(SE, SA), { empty: right(SA.empty) });
}
