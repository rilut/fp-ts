...
function getShow(SL, SA) {
    return {
        show: fold(function (l) { return "left(" + SL.show(l) + ")"; }, function (a) { return "right(" + SA.show(a) + ")"; }, function (l, a) { return "both(" + SL.show(l) + ", " + SA.show(a) + ")"; })
    };
}
 ...
 * @since 2.0.0
 */
function toTuple(l, a) {
    return function (fa) { return (isLeft(fa) ? [fa.left, a] : isRight(fa) ? [l, fa.right] : [fa.left, fa.right]); };
}
...
function fromOptions(fl, fa) {
    return Option_1.isNone(fl)
        ? Option_1.isNone(fa)
            ? Option_1.none
            : Option_1.some(right(fa.value))
        : Option_1.isNone(fa)
            ? Option_1.some(left(fl.value))
            : Option_1.some(both(fl.value, fa.value));
}
