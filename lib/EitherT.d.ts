...
export interface EitherM<M> extends ApplicativeComposition02<M, URI> {
    ...
    readonly fold: <L, A, R>(onLeft: (l: L) => HKT<M, R>, onRight: (a: A) => HKT<M, R>) => (ma: EitherT<M, L, A>) => HKT<M, R>;
...
export interface EitherM1<M extends URIS> extends ApplicativeComposition12<M, URI> {
    ...
    readonly fold: <L, A, R>(onLeft: (l: L) => Type<M, R>, onRight: (a: A) => Type<M, R>) => (ma: EitherT1<M, L, A>) => Type<M, R>;
...
export interface EitherM2<M extends URIS2> extends ApplicativeComposition22<M, URI> {
    ...
    readonly fold: <LM, L, A, R>(onLeft: (l: L) => Type2<M, LM, R>, onRight: (a: A) => Type2<M, LM, R>) => (ma: EitherT2<M, LM, L, A>) => Type2<M, LM, R>;
    ...
    readonly orElse: <LM, L, A, N>(f: (l: L) => EitherT2<M, LM, N, A>) => (ma: EitherT2<M, LM, L, A>) => EitherT2<M, LM, N, A>;
    ...
    readonly bracket: <LM, L, A, B>(acquire: EitherT2<M, LM, L, A>, use: (a: A) => EitherT2<M, LM, L, B>, release: (a: A, e: Either<L, B>) => EitherT2<M, LM, L, void>) => EitherT2<M, LM, L, B>;
}
/**
 * @since 2.0.0
 */
