 ...
 * @since 2.0.0
 */
function foldL(as, onNil, onCons) {
    return isEmpty(as) ? onNil() : onCons(as[0], as.slice(1));
}
exports.foldL = foldL;
/**
 ...
 * @since 2.0.0
 */
function foldr(as, onNil, onCons) {
 ...
 * @since 2.0.0
 */
function foldrL(as, onNil, onCons) {
    return isEmpty(as) ? onNil() : onCons(as.slice(0, as.length - 1), as[as.length - 1]);
}
exports.foldrL = foldrL;
/**
 ...
 * @since 2.0.0
 */
function dropWhile(as, predicate) {
    var i = spanIndexUncurry(as, predicate);
    var l = as.length;
    var rest = Array(l - i);
    for (var j = i; j < l; j++) {
 ...
 * @since 2.0.0
 */
function findLastIndex(as, predicate) {
    var len = as.length;
    for (var i = len - 1; i >= 0; i--) {
        if (predicate(as[i])) {
            return Option_1.some(i);
 ...
 */
function zip(fa, fb) {
    return zipWith(fa, fb, function_1.tuple);
}
exports.zip = zip;
/**
...
function sortBy(ords) {
    return fold(ords, Option_1.none, function (head, tail) { return Option_1.some(sortBy1(head, tail)); });
}
 ...
 * @since 2.0.0
 */
function sortBy1(head, tail) {
    return sort(tail.reduce(Ord_1.getSemigroup().concat, head));
}
 ...
 * @since 2.0.0
 */
function chunksOf(as, n) {
    return isOutOfBound(n - 1, as) ? [as] : chop(as, function (as) { return splitAt(n, as); });
}
...
function comprehension(input, f, g) {
    var go = function (scope, input) {
        if (input.length === 0) {
            ...
            return chain(input[0], function (x) { return go(snoc(scope, x), input.slice(1)); });
        }
    };
...
var ap = function (fab, fa) {
    return flatten(map(fab, function (f) { return map(fa, f); }));
};
...
var sequence = function (F) { return function (ta) {
    return reduce(ta, F.of(zero()), function (fas, fa) { return F.ap(F.map(fas, function (as) { return function (a) { return snoc(as, a); }; }), fa); });
}; };
var compact = function (as) { return filterMap(as, function_1.identity); };
var filter = function (as, predicate) {
    return as.filter(predicate);
};
var filterMap = function (as, f) {
...
var partitionWithIndex = function (fa, predicateWithIndex) {
    var left = [];
    var right = [];
    for (var i = 0; i < fa.length; i++) {
        var a = fa[i];
...
var filterWithIndex = function (fa, predicateWithIndex) {
...
exports.array = {
    ...
    sequence: sequence,
    zero: zero,
    alt: function (fx, f) { return function_1.concat(fx, f()); },
    extend: extend,
    wither: wither,
    wilt: wilt,
